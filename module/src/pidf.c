...
int xmlParseLocationInfo(xmlNodePtr node, p_loc_t loc)
{
    ...
    if (content) {
        ...
        snprintf(loc->latitude, strlen((char *)bufLat) + 1, "%s", (char *)bufLat);

        loc->longitude = (char *)pkg_malloc(strlen((char *)bufLon) + 1);
        snprintf(loc->longitude, strlen((char *)bufLon) + 1, "%s", (char *)bufLon);
...
char *xmlParseFindServiceResponse(xmlNodePtr node, const char *name, int *len)
{
	...
	cnt = (char *)xmlNodeGetNodeContentByName(cur, name, NULL);
	*len = strlen(cnt);
